...
class CRCWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Kalkulator CRC")
        self.setup_ui()

    def setup_ui(self):
        layout = QVBoxLayout()

        sequence_label = QLabel("Bajty ramki:")
        self.sequence_regexp = QRegularExpression("[0-9A-F]{512}}")
    ...
    def calculate_crc(self):
        ...
        if len(data) % 2 != 0 or data == '' or self.repeat_input.text() == '':
        ...
        for _ in range(repeats):
            crc = backend.calculate_crc_fast(hex_data)
        ...
        self.result_label3.setText("CRC: {:04X}".format(crc))
